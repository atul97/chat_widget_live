...
function response_live() { 
    setInterval(function() {
        setTimeout(() => {
            hideBotTyping();

        $.ajax({
            url: rasa_server_url2,
            type: "GET",
            contentType: "application/json",
            // data: JSON.stringify({ "message":message, "receiver": "Admin", sender: sender_id }),
            
            success(liveChatResponse, status) {
                console.log(liveChatResponse[0]['message']);
                console.log("Response from Live User: ", liveChatResponse, "\nStatus: ", status);
                const LiveResponse = `<img class="botAvatar" src="./static/img/sara_avatar.png"/><p class="botMsg">${liveChatResponse[0]['message']}</p><div class="clearfix"></div>`;

                $(LiveResponse).appendTo(".chats").hide().fadeIn(1000);
                // setBotResponse(botResponse);
            },
            error(xhr, textStatus) {
    
                // if there is no response from rasa server, set error bot response
                $("Error2").appendTo(".chats").hide().fadeIn(1000);
                // setBotResponse("error");
                console.log("Error from live end: ", textStatus);
            },
        });
        console.log(url);
    });
...
function setBotResponse(response) {
    ...
    if (present_url == rasa_server_url2)
    {
        response_live()
    //     setTimeout(() => {
    //         hideBotTyping();
...
function send(message) {
    ...
    if (present_url == rasa_server_url2)
    {
        ...
        $.ajax({
            ...
            type: "POST",
            contentType: "application/json",
            data: JSON.stringify({"message":message,"receiver": "Admin","sender":"admin123"}),
            
            success(botResponse, status) {
                console.log("Response from User: ", botResponse, "\nStatus: ", status);
